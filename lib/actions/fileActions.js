...
export async function getSignedURLForCertificates(
  type,
  size,
  checksum,
  fileName,
  ...
  // Update the student's certificate URL in the database
  const addCert = await prisma.certificate.create({
    data: {
      certificateUrl: signedURL.split("?")[0],
      student: {
        connect: {
          id: studentId,
        },
